...
public class SftpDaoImpl implements SftpDao {
  private static final Logger LOG = LoggerFactory.getLogger(SftpDaoImpl.class);
  ...
  @Override
  public void enviarArchivo(String localFilenamePath, String remoteFilenamePath) {
    try {
      if (getConnection()) {
        LOG.info("Enviando archivo");
        channelSftp.put(localFilenamePath, remoteFilenamePath);
        LOG.info("Envio completada");
      }
    }
    catch (Throwable e) {
      LOG.info("Insidencia al enviar archivo");
      throw new Error500(List.of("Insidencia al enviar archivo"), e);
    }
    finally {
      closeConnection();
    }
  ...
  @Override
  public void descargarArchivo(String remoteFilenamePath, String localFilenamePath) {
    try {
      if (getConnection()) {
